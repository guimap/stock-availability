...
class StockService {
  ...
  async getAverageOrders (uf, city) {
    ...
    const sum = orderByMonth.reduce((ammount, orderMonth) => {
      ammount += orderMonth.total
      return ammount
    }, 0)
  ...
  async getAllStockAvailability () {
    ...
    const pendingPromises = allStocks.map(async (stock) => {
      try {
        ...
        console.log(`Error on ${stock.uf} - ${stock.city}`)
        return {}
      }
    })
    const results = await Promise.all(pendingPromises)
    //  Filtra resultados que tenham alert
    return results.filter(result => result.alert)
  }
}
